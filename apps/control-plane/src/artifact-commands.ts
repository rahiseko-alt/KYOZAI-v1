export type ArtifactCommand =
  ...
  | { command: "updateMetadata"; artifactId: string; metadataJson: string }
...
export function parseArtifactCommand(value: unknown): ArtifactCommand {
  ...
  if (x.command === "updateMetadata") { const metadataJson = text(x.metadataJson); try { JSON.parse(metadataJson); } catch { throw new ArtifactCommandError("BAD_COMMAND"); } return { command: "updateMetadata", artifactId: text(x.artifactId), metadataJson }; }
...
export async function executeArtifactCommand(db: D1Database, x: ArtifactCommand) {
  ...
  if (x.command === "updateMetadata") {
    const result = await db.prepare("UPDATE artifacts SET metadata_json = ? WHERE id = ? AND lifecycle IN ('draft', 'validated')").bind(x.metadataJson, x.artifactId).run();
    if (result.meta.changes !== 1) throw new ArtifactCommandError("CONFLICT");
    return { artifactId: x.artifactId };
  }
