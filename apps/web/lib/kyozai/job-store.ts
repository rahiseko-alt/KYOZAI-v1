...
export function snapshotFromRows(job: Record<string, unknown>, revision: Record<string, unknown>, stages: Array<Record<string, unknown>>, artifacts: Array<Record<string, unknown>>): KyozaiJobSnapshot {
  ...
  return {
    ...
    stages: stages.map((stage): StageLedgerEntry => ({
      ...
      inputArtifactIds: stringArray(stage.input_artifact_ids ?? stage.input_artifact_ids_json),
      outputArtifactIds: stringArray(stage.output_artifact_ids ?? stage.output_artifact_ids_json),
