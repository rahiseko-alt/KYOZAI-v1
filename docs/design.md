...
## 2026-08-29 個人PWAへの製品境界変更

利用者の明示指示により、SaaS化に関する実装・課金・テナント管理・不特定多数向け運用は凍結する。完成対象は、利用者本人が自分の端末と自分のAPIキーで利用するPWAとする。多人数用のCloudflare Access所有者分離、共有quota、R2 subscription、SaaS向け運用SLOは再開時まで扱わない。

PWAとしてもAPIキーをブラウザーへ露出させず、未設定・上限不明・成果物保存先なしでは生成を拒否する。個人利用の範囲で完成させるが、SaaS相当の可用性・課金・マルチテナント保証を提供する主張はしない。
...
## 11. 2026-08-13 画像生成モデルは生成ごとに確認する

画像生成モデルをAPPの既定値として固定しない。教材の生成または画像を伴う修正を開始するたびに、
利用者が審査済み候補から明示選択する。未選択の生成はUIとAPIの両方で拒否する。

- 候補は`gemini-3.1-flash-lite-image`、`gemini-3.1-flash-image`、`gpt-image-2` mediumとする。
- 選択したモデルIDをimage prompt、image validation、manifestへ記録する。
- APIキーはサーバー側だけに置き、利用者へキー入力を求めない。
- 工程、profile、凍結条件、画像QA、ページ単位再生成、納品条件はモデル選択にかかわらず同一とする。

従来の「モデルはサーバー側で固定する」は教材構成・修正用の`gpt-5.5`に限る。画像モデルまで
固定する解釈は誤りだったため、品質、速度、費用の選択を生成ごとに利用者へ戻す。

---

## 12. 2026-08-26 工程同等化の完了境界と実行正本

従来のフェーズ計画は、途中で発見した問題の追跡と、実fixtureの完走を近づけない基盤実装を
止められなかった。そのため、実行正本を
`docs/skill-app-parity-execution-plan-2026-08-26.md` へ変更する。

完了境界は、直接入力、長文PDF、字幕付きYouTube、参考デザイン、自然文修正の5fixtureが、
正本SkillとAPPで工程・停止条件・品質基準・成果物契約を満たし、実環境の外部証拠を残した状態とする。
この完了前にProduction生成を再開しない。

---

## 13. 2026-08-27 運用費0円と生成費の分離

利用者は、サーバー、DB、Storage、定期実行を含む運用費を0円に固定した。AI本文・画像・画像QAの
provider利用料は、利用者ごとの実費として予約・確定・上限停止を行う。運用者が有料Vercel planや
常時稼働workerへ切り替えることは、この決定に反する。

- Vercelの有料Cronは使わない。Vercel配備設定にCronを置かない。
- dispatcherとcleanupは、無料Supabase project内の`pg_cron`／`pg_net`から認証済みVercel endpointを起動する。
