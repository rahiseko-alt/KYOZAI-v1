...
- 本文stream停止種別の分類（G1-PWA-CONTENT-STREAM-003）: 親Gateは同じG1個人PWA公開例外。004を配備後も返答前接続失敗の分類記録が出ず、`analysis`の504はHTTP応答受信後のSSE本文処理で起きていることが判明した。SSEのmissing body、timeout、JSON不正、`error`イベント、read失敗だけを安全なenumへ分類し、本文・上流エラーメッセージ・APIキーを記録しない。原因が確定するまで再送、モデル、画像、品質工程は変更しない。受入条件は、各fixtureが期待するenumのみを返し、Production最小fixtureの停止種別をVercelログで確認できること。テストは停止stream／errorイベントfixture、既存の接続失敗fixture、型検査・lint・全test・build・smoke・E2Eを使用する。
- 本文の未分類provider状態の分類（G1-PWA-CONTENT-STATE-001）: 親Gateは同じG1個人PWA公開例外。STREAM-003を配備したProduction最小fixtureでも`analysis`が504となり、既存の接続・stream分類記録が出なかった。`provider_result_unavailable`の安全な状態名、または未分類例外の型名だけを、工程名・試行番号・経過時間とともに記録する。入力・本文・上流メッセージ・APIキー・状態の任意値は記録しない。受入条件は、次の同一fixtureで停止経路が1つの固定enumまたは安全な例外型として観測できること。原因が確定するまで再送・モデル・画像・品質工程は変更しない。
- 個人PWA本文の非stream応答化（G1-PWA-CONTENT-TRANSPORT-001）: 親Gateは同じG1個人PWA公開例外。ProductionでSSE本文処理の分類記録まで到達せず`analysis`が504となる一方、個人PWAの画面はストリーミング表示を使わず完成した構造化JSONだけを必要とする。直接個人PWAの未追跡呼出しだけをResponses APIの通常JSON応答へ切り替え、追跡済みSaaS／workflow経路はSSEのまま維持する。モデル、プロンプト、schema、品質工程、画像生成は変更しない。受入条件は、直接PWAのrequestが`stream:false`で構造化packageを完走し、追跡済み経路は`stream:true`を維持すること。テストは既存のJSON／SSE fixture、型検査、lint、CI、Production最小fixtureを使用する。
