...
- 次の着手: create commandのD1実fixture（許可・拒否・競合）を追加し、webのstate I/Oをgateway clientへ
  ...
  local D1 fixtureでclaim responseとjob/stageの同時running遷移、pass後のstarted_at保持・completed_at設定・lease解除、fail/retryのattempt遷移を確認した。artifactのdraft登録・checksum validation・final昇格commandを追加した。次はR2 streaming I/OとVercel readback hash検証を接続する。Webにはserver-onlyのcontrol-plane clientを追加済みで、tokenをブラウザーへ渡さず、
